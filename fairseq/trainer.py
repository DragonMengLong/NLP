...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        if self.args.fp16:
            if self.cuda and torch.cuda.get_device_capability(0)[0] < 7:
                print('| WprintARNING: your device does NOT support faster training with --fp16, '
    ...
    def train_step(self, samples, dummy_batch=False):
        ...
        try:
            ...
            if hasattr(self.meters['gnorm'].sum,'device') :
                if self.meters['gnorm'].sum.device != 'cuda:0': 
                    self.meters['gnorm'].sum = self.meters['gnorm'].sum.to(torch.device('cuda:0'))
            ###################
