...
def get_parser(desc, default_task='translation'):
    ...
    parser.add_argument('--learn-multiloss-w', default=False, action='store_true',
                        help=('should model learn weight between multi-loss'))
    parser.add_argument('--depth-select-target', default='None',
                        choices=['None', 'seq_LL', 'seq_cor', 'tok_LL', 'tok_cor', 'tok_confi'],
                        help='choose a depth prediction target')
    parser.add_argument('--depth-alpha', default=1.0, type=float,
                        help='weight of depth-select-method loss')
    parser.add_argument('--depth-select-method', default='seq_depth',
                        choices=['None', 'seq_depth', 'token_depth_M', 'token_depth_G'],
                        help='choose a depth prediction method')
    parser.add_argument('--layer', default=0, type=int,
                        help='choose a fixed decoder layer, 0 for depth adaptive')
    parser.add_argument('--classifier', default='single',
                        choices=['single', 'multiple'],
                        help='use a single classifier or multiple classifier in decoder')
    #################
...
def add_optimization_args(parser):
    ...
    ###############
    group.add_argument('--depth-lambda', default=0, type=float,
                       help='bigger lambda expect to receive a smaller n')
    ###############
