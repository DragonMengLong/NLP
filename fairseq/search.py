...
class BeamSearch(Search):
    ...
    def step(self, step, lprobs, scores):
        ...
        if step == 0:
            ...
            # only the first beam
            '''
            ::表示，间隔beam取一次，由于第一次生成时候，还没有beam个候选
            '''
            lprobs = lprobs[:, ::beam_size, :].contiguous()
        else:
            # make probs contain cumulative scores for each hypothesis
            # print(lprobs.size())
            # print(scores[:, :, step - 1].unsqueeze(-1).size())
            '''
            lprobs是全词表的概率，score是已生成的序列概率和，将score加到lprobs上，
            得到全词表的概率，下面将会根据beam大小，选择出beam个保留
            '''
            lprobs.add_(scores[:, :, step - 1].unsqueeze(-1))

        # print(lprobs.view(bsz, -1).size())
        torch.topk(
            lprobs.view(bsz, -1), #输入2维数据，第二维是beam*V，在第二维选beam个最大的
            k=min(
                # Take the best 2 x beam_size predictions. We'll choose the first
                # beam_size of these which don't predict eos to continue with.
                beam_size * 2, #应该是有一半是eos，这里是为啥能保证有一半？？？？
                lprobs.view(bsz, -1).size(1) - 1,  # -1 so we never select pad
            ),
            out=(self.scores_buf, self.indices_buf),
        )
        '''
        scores_buf存的是每一行中topk的结果，也就是beam*V中的topk，
        indices_buf中每一行保存的是scores_buf在lprobs的对应索引
        '''
        self.beams_buf = torch.div(self.indices_buf, vocab_size) # RuntimeError: result type Float can't be cast to the desired output type Long
