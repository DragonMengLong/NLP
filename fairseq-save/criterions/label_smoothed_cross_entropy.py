...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if len(dec_layers_out) != 0:
            for item in dec_layers_out:
                ...
                if self.depth_target is not "None":
                    if self.depth_target is "seq_LL":
                        seq_logP_list.append(logP + self.depth_lambda * depth_num)
                    elif self.depth_target is "seq_cor":
                        seq_logP_list.append(logP + -self.depth_lambda * depth_num)
        multi_depth = None
        seq_loss["func_q"] = dict_n["pred"]
        ...
        if self.depth_target is not "None":
            for i in range(len(seq_logP_list)):
                if i == 0:
                    ...
                    multi_depth = torch.cat((multi_depth, seq_logP_list[i]), 1)
            if self.depth_target is "seq_LL":
                _, depth_index = multi_depth.min(-1, keepdim=True)
                seq_loss["func_q_star"] = depth_index.squeeze(-1)
                # depth_index.requires_grad = False
            elif self.depth_target is "seq_cor":
            ...
            depth_loss = self.dep_cri(seq_loss["func_q"], seq_loss["func_q_star"])
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        ###########
        beam_batch_size, lenght, word_V = lprobs.size()
        # print(lprobs.size())
        ###########

        lprobs = lprobs.view(-1, lprobs.size(-1)) #变为(batch*beam*len)*V
        target = model.get_targets(sample, net_output).view(-1, 1) #将target变为 (batch*beam*len)*1
        non_pad_mask = target.ne(self.padding_idx) # padding_idx=1，和之前的维度一样
        # 取lprobs在target位置上的值
        ...
        if self.depth_target == "seq_LL":
            ...
            lp_final_bb = lp_bb_nopad.sum(-1, keepdim=True)
        elif self.depth_target == "seq_cor":
            model_pred, _ = lprobs.max(-1, keepdim=True)
            correct_num = nll_loss_fake == model_pred
            correct_num_bb = correct_num.view(beam_batch_size, lenght, -1).squeeze(-1)
            lp_final_bb = correct_num_bb.sum(-1, keepdim=True)
        else:
            lp_final_bb = None
        ############
